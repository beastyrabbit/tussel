import {
  ...
  isPlainObject,
  type MasterSpec,
  PATTERN_SIGNAL_IDENTIFIERS,
  renderValue,
  type SceneSpec,
  STRUDEL_MAPPED_METHODS,
} from '@tussel/ir';
...
/**
 * Methods with a direct Strudel mapping, derived from the shared
 * PATTERN_METHOD_REGISTRY in @tussel/ir (entries flagged strudelSupported).
 */
const BUILTIN_METHODS: ReadonlySet<string> = STRUDEL_MAPPED_METHODS;

/** Zero-argument signal call identifiers (`sine()`, `rand()`, ...). */
const ZERO_ARG_IDENTIFIERS: ReadonlySet<string> = PATTERN_SIGNAL_IDENTIFIERS;
